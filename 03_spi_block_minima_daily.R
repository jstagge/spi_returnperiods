...
write_output_path <- file.path(output_path, "daily")
...
n_years <- 500000
#n_years <- 100
accum_list <- c(15, 30, 60, 90, 182, 274, 365, 730)
...
for (j in seq(1,n_accum)){
  ...
  ### Read in
  for (k in seq(1,20)){
    ann_min_temp <- readRDS(file.path(spi_folder, paste0("ann_min_", accum_j, "_fold",k, ".rds")))
...
for (j in seq(1,n_accum)){
  ...
  ###  Load Results
  ###########################################################################
  spi_folder <- file.path(write_output_path, paste0("daily/spi_", accum_j))
  ...
  for (k in seq(1,20)){
    ...
    ann_min_temp <- ann_min_temp %>%
      ...
      ### Calculate the sample L moments
      sample_mom <- lmoms(0-ann_min_temp$ann_min)
      sample_mom

  ### Combine into a dataframe
    l_mom_temp <- data.frame(accum_period = accum_j, Moment = c("l_1", "l_2", "l_3", "l_4", "t_3", "t_4")) %>%
      	mutate(Population = c(sample_mom$lambdas[1:4], sample_mom$ratios[3:4]))	%>%
        pivot_wider(names_from = "Moment", values_from = "Population") %>%
        mutate(fold = k)
    ...
    if(j == 1 & k ==1){
      ...
      l_mom_df <- l_mom_df %>%
        bind_rows(l_mom_temp)
    }
  }
}

saveRDS(l_mom_df, file.path(write_output_path, paste0("daily/l_mom_folds.rds")))
